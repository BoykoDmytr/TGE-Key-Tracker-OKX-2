...
export interface DecodedSetTime {
  startTime: number;
  duration: number;
}

// Decodes setTime(uint256 _startTime, uint256 _duration) calldata.
export function decodeSetTime(input: string): DecodedSetTime | null {
  if (!input || input.length < 10) return null;
  if (input.slice(0, 10).toLowerCase() !== SETTIME_SELECTOR) return null;
  ...
  try {
    ...
    if (!Number.isFinite(startTime) || !Number.isFinite(duration)) return null;
